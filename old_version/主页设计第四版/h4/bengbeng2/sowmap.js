...
function YSlide(config) {   //动画
	...
	if (config.direction == direction) { //根据方向自动滚动
		...
		function autoslide() {       
			if (bool) {        
				img = img + 1;        
				if (img == config.num) {
					eval('startMove(oBoxUL,{' + direction + ':-(oBox_px*img)},function(){oBoxUL.style.' + direction + '=0})');
					img = 0;
				}        
				else {
	...
	if (config.lr_btn.show) { //如果config.lr_btn.show为true，创建左右点击按钮
		...
		l_btn.onclick = function () {  //点击左按钮事件          
			...
			img = img - 1      
			if (img < 0) {
				eval('oBoxUL.style.' + direction + '= -(oBox_px*config.num)+"px";img = config.num-1;startMove(oBoxUL,{' + direction + ':-(oBox_px)*(config.num-1)})')
			}      
			else {
				eval('startMove(oBoxUL,{' + direction + ':-(oBox_px*img)})');
			}      
